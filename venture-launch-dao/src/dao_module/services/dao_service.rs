...
use crate::multisig_utils::base_multisig::{
    BaseMultisig, BaseMultisigCreateArgs, BaseMultisigInitArgs,
};
...
async fn get_ba_keypair() -> Result<Keypair, String> {
    ...
    let private_key_vec: Vec<u8> = private_key_str
        .split(',')
...
fn get_rpc_client() -> Result<RpcClient, String> {
    ...
    Ok(RpcClient::new(
        std::env::var("DEFAULT_RPC_CLIENT")
...
async fn create_base_multisig(create_key: &Keypair) -> Result<BaseMultisig, String> {
    ...
    let creator_keypair: Keypair = get_ba_keypair()
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    ...
    let mut transaction = Transaction::new_with_payer(&[ix], Some(&creator_keypair.pubkey()));

    let recent_blockhash = multisig
        .get_rpc_client()
    ...
    let associated_token_address =
        get_associated_token_address(&multisig.get_vault_pda(), &token_mint);
    let account_info = multisig
        .get_rpc_client()
...
async fn get_base_multisig(multisig_pda: Pubkey) -> Result<BaseMultisig, String> {
    ...
    let creator_keypair = get_ba_keypair()
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
...
pub async fn create_dao() -> Result<String, String> {
    ...
    let multisig = create_base_multisig(&create_key)
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    ...
    let mut tx = multisig
        .transaction_create_multisig(&[], 1, 0, &create_key)
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let recent_blockhash = multisig
        .get_rpc_client()
    ...
    Ok(format!(
        ...
        \"vault_pda\":  \"{}\",
        \"threshold\":  \"{}\"",
        multisig.get_multisig_pda().to_string(),
        multisig.get_vault_pda().to_string(),
        multisig.get_threshold().await.unwrap()
    ))
}
...
pub async fn add_member(
    ...
    let multisig_pda =
        Pubkey::from_str(&multisig_pda).map_err(|err| format!("\"msg\": \"{err}\""))?;
    let multisig = get_base_multisig(multisig_pda)
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;

    let multisig: Arc<&dyn BusinessAnalystMultisigTrait> = Arc::new(&multisig);

    let new_member_pubkey =
        Pubkey::from_str(pubkey.as_str()).map_err(|err| format!("\"msg\": \"{err}\""))?;
    ...
    let mut tx = multisig
        .transaction_add_member(creator_keypair.pubkey(), new_member)
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    ...
    let ix_prpose = multisig
        .instruction_proposal_create(creator_keypair.pubkey())
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let ix_approve = multisig
        .instruction_proposal_approve(creator_keypair.pubkey())
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let ix_exec = multisig
        .instruction_config_transaction_execute(creator_keypair.pubkey())
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let mut tx = multisig
        .get_transaction_from_instructions(
            creator_keypair.pubkey(),
            &[ix_prpose, ix_approve, ix_exec],
        )
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let recent_blockhash = multisig
    ...
    Ok(format!("\"member\":  \"{}\"", new_member_pubkey))
}

pub async fn remove_member(multisig_pda: String, pubkey: String) -> Result<String, String> {
    dotenv().ok();
    ...
    let multisig: Arc<&dyn BusinessAnalystMultisigTrait> = Arc::new(&multisig);
    let old_member_pubkey =
        Pubkey::from_str(pubkey.as_str()).map_err(|err| format!("\"msg\": \"{err}\""))?;

    let ix_remove_member = multisig
        .instructions_remove_member(creator_keypair.pubkey(), old_member_pubkey)
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let mut tx = multisig
        .get_transaction_from_instructions(creator_keypair.pubkey(), &[ix_remove_member])
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let recent_blockhash = multisig
        .get_rpc_client()
    ...
    Ok(format!("\"member\":  \"{}\"", old_member_pubkey))
}

pub async fn change_threshold(multisig_pda: String, new_threshold: u16) -> Result<String, String> {
    dotenv().ok();
    ...
    let ix_change_threshold = multisig
        .instruction_change_threshold(creator_keypair.pubkey(), new_threshold)
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let mut tx = multisig
        .get_transaction_from_instructions(creator_keypair.pubkey(), &[ix_change_threshold])
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let recent_blockhash = multisig
        .get_rpc_client()
    ...
    Ok(format!("\"new_threshold\":  \"{}\"", new_threshold))
}

pub async fn execute_proposal(multisig_pda: String) -> Result<String, String> {
    dotenv().ok();
    ...
    let mut tx = multisig
        .transaction_config_transaction_execute(creator_keypair.pubkey())
        .await
        .map_err(|err| format!("\"msg\": \"{err}\""))?;
    let recent_blockhash = multisig
        .get_rpc_client()
    ...
    Ok(format!("\"propose\":  \"success\""))
}

pub async fn vote(multisig_pda: String, voter: String, vote: String) -> Result<String, String> {
    dotenv().ok();
    ...
    let mut tx = match vote.as_str() {
        "Cancel" => multisig
            .transaction_proposal_cancel(creator_keypair.pubkey())
            .await
            .unwrap(),
        "Approve" => multisig
            .transaction_proposal_approve(creator_keypair.pubkey())
            .await
            .unwrap(),
        vote => {
...
pub async fn withdraw(
    ...
    if is_execute == true {
        let mut tx = multisig
            .transaction_vault_transaction_execute(creator_keypair.pubkey(), receiver, amount)
            .await
            .unwrap();
        let recent_blockhash = multisig
            .get_rpc_client()
    ...
    let mut tx = multisig
        .transaction_transfer_from_vault(creator_keypair.pubkey(), receiver, amount)
        .await
        .unwrap();
    let recent_blockhash = multisig
        .get_rpc_client()
    ...
    let ix_approve = multisig
        ...
        .unwrap();
    let mut tx = multisig
        .get_transaction_from_instructions(creator_keypair.pubkey(), &[ix_propose, ix_approve])
        .await
        .unwrap();
    let recent_blockhash = multisig
        .get_rpc_client()
